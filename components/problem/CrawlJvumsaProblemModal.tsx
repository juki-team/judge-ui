    ...
    <>
      ...
      <ButtonLoader
        ...
        onClick={async (setLoaderStatus) => {
          ...
          pushRoute(jukiSettings.ROUTES.problems().view({ key: `${judge.keyPrefix}-${key}` }));
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} closeWhenClickOutside closeWhenKeyEscape closeIcon>
      <div className="jk-col gap jk-pg-md">
        <FetcherLayer<ContentResponseType<JudgeDataResponseDTO>> url={jukiSettings.API.judge.getData({ params: { key: Judge.JV_UMSA as string } }).url}>
          {({ data: { content } }) => (
