...
import { CommandsFunctions, CommandsObject } from './types';

const keys: CommandsFunctions = {
...
export const getCommands = (text: string): [CommandsObject, string] => {
  const X = text.split('\n');
  const Y = X[0].split(' ');
  const commands = Y[0].split('\\');
  const commandsObject: CommandsObject = {};
  let i = 0;
  if (commands[0] === '') {
